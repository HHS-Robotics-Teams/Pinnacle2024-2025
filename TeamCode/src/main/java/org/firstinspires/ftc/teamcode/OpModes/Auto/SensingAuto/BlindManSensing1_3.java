    ...
    public void loop() {
        ...
        switch (autoState) {
            case PRELOAD:
                ...
                testTimer.reset();
                autoState = BlindManSensing1_2.AutoState.DRIVE;
                break;
            case DRIVE:
                slideMotor.setTargetPosition(SlideHighChamber + 12);
                if (testTimer.seconds() >= 1.5) {
                    dunkTimer.reset();
                    autoState = BlindManSensing1_2.AutoState.DUNK;
                    break;
                }
                break;
            case DUNK:
                ...
                if (dunkTimer.seconds() >= 0.3) {
                    ...
                    goBackTimer.reset();
                    autoState = BlindManSensing1_2.AutoState.GO_BACK;
                    break;
                }
                break;
            case GO_BACK:
                if (goBackTimer.seconds() > 0.3) {
                    ...
                    autoState = BlindManSensing1_2.AutoState.PICK_UP_FIRST_SAMPLE;
                    break;
                }
                break;
            case PICK_UP_FIRST_SAMPLE:
                ...
                autoState = BlindManSensing1_2.AutoState.EXTEND;
                break;
            case EXTEND:
                intakeElbowServo.setPosition(ElbowLeft);
                if ((Math.abs(drive.getPoseEstimate().getX() - 20) <= 2.5) && (Math.abs(drive.getPoseEstimate().getY() + 39) <= 2.5)
                    ...
                    grabTimer.reset();
                    autoState = BlindManSensing1_2.AutoState.MOVE_GRADUALLY;
                    break;
                }
                break;
            case MOVE_GRADUALLY:
                if ((slideMotor.getCurrentPosition() >= 195) && (Math.abs(drive.getPoseEstimate().getHeading() - targetHeading) < Math.toRadians(6))) {
                    if (distance < OldDetectedDistance ) {
                        ...
                        autoState = BlindManSensing1_2.AutoState.GRAB_FIRST_SAMPLE;
                        break;
            ...
            case GRAB_FIRST_SAMPLE:
                if (grabTimer.seconds() >= 0.25) {
                    ...
                    if (grabTimer.seconds() >= 0.4) {
                        slideMotor.setTargetPosition(0);
                        autoState = BlindManSensing1_2.AutoState.TURN_TO_OBSERVATION;
                        break;
                    }
                }
                break;
            case TURN_TO_OBSERVATION:
                ...
                intakeElbowServo.setPosition(ElbowLeft);
                grabTimer.reset();
                autoState = BlindManSensing1_2.AutoState.EXTEND_TO_OBSERVATION;
                break;
            case EXTEND_TO_OBSERVATION:
                if ((Math.abs(drive.getPoseEstimate().getX() - 20) <= 3) &&
                    ...
                    if (slideMotor.getCurrentPosition() >= 240){
                        ...
                        drive.update();
                        OldDetectedDistance = detectedDistance;
                        autoState = BlindManSensing1_2.AutoState.MOVE_GRADUALLY_SECOND_SAMPLE;
                        break;
                    }
                }
                break;
            case MOVE_GRADUALLY_SECOND_SAMPLE:
                if ((Math.abs(tiltMotor.getCurrentPosition() - 515) <= TiltTickThreshold) && (Math.abs(drive.getPoseEstimate().getHeading() - targetHeading_TWO) < Math.toRadians(6))) {
                    if (distance < OldDetectedDistance) {
                        ...
                        tiltMotor.setPower(0);
                        autoState = BlindManSensing1_2.AutoState.GRAB_SECOND_SAMPLE;
                        break;
                    } else if (isSlideIncrementing) {
                        slideMotor.setTargetPosition(slideMotor.getCurrentPosition() + 15);
                        isSlideIncrementing = false; // Wait before the next increment
            ...
            case GRAB_SECOND_SAMPLE:
                if (grabTimer.seconds() > 0.3 ){
                    tiltMotor.setPower(0);
                    grabAgainTimer.reset();
                    autoState = BlindManSensing1_2.AutoState.GO_OBSERVATION_AGAIN;
                }
                break;
            case GO_OBSERVATION_AGAIN:
                if (grabAgainTimer.seconds() >= 0.3) {
                    ...
                    if (grabAgainTimer.seconds() >= 0.5) {
                        ...
                        drive.update();
                        grabTimer.reset();
                        autoState = BlindManSensing1_2.AutoState.DROP_SPECIMEN_AGAIN;
                    }
                }
                break;
            case DROP_SPECIMEN_AGAIN:
                if (grabTimer.seconds() >= 0.8){
                    ...
                    if (slideMotor.getCurrentPosition() >= 280) {
                        intake_claw_servo.setPosition(Claws_open);
                        grabAgainTimer.reset();
                        autoState = BlindManSensing1_2.AutoState.RETRACT;
                        break;
                    }
                }
                break;
            case RETRACT:
                ...
                slideMotor.setTargetPosition(0);
                autoState = BlindManSensing1_2.AutoState.LIFT_SLOWLY;
                break;
            case LIFT_SLOWLY:
                intakeWristServo.setPosition(WristRight);
                intakeElbowServo.setPosition(ElbowLeft);
                ...
                if (grabAgainTimer.seconds() >= 0.3 && Math.abs(tiltMotor.getCurrentPosition() - 560) <= TiltTickThreshold) {
                    ...
                    isSlideIncrementing = true;
                    testTimer.reset();
                    autoState = BlindManSensing1_2.AutoState.GRAB_FIRST_SPECIMEN;
                    break;
                }
                break;
                ...
                if (testTimer.seconds() > .5){
                    if (Math.abs(slideMotor.getCurrentPosition() - 460) <= SlideTickThreshold) {
                        if ((distance < 2.0) && tiltMotor.getCurrentPosition() >= 570) {
                            intake_claw_servo.setPosition(Claws_closed);
                            grabTimer.reset();
                            autoState = BlindManSensing1_2.AutoState.LIFT_FIRST_SPECIMEN;
                            break;
            ...
            case LIFT_FIRST_SPECIMEN:
                if (grabTimer.seconds() >= 0.4){
                    ...
                    if (tiltMotor.getCurrentPosition() >= 705){
                        ...
                        intakeWristServo.setPosition(WristRight);
                        dunkTimer.reset();
                        autoState = BlindManSensing1_2.AutoState.SCORE_FIRST_SPECIMEN;
                        break;
                    }
                }
                break;
                    ...
                    if ((Math.abs(drive.getPoseEstimate().getX() - 24) <= 2) && (Math.abs(drive.getPoseEstimate().getY() - 0) <= 2)
                        ...
                        if (dunkTimer.seconds() > 1.8) {
                            if (tiltMotor.getCurrentPosition() >= 1300) {
                                ...
                                grabTimer.reset();
                                autoState = BlindManSensing1_2.AutoState.LET_GO_OF_SPECIMEN;
                                break;
                            }
                        }
                    }
            ...
            case LET_GO_OF_SPECIMEN:
                if (grabTimer.seconds() >= 1.2) {
                    ...
                    if (dunkTimer.seconds() >= .8) {
                        ...
                        intakeWristServo.setPosition(WristCenter);
                        autoState = BlindManSensing1_2.AutoState.GO_TO_OBSERVATION_AGAIN;
                        break;
                    }
                }
                break;
            case GO_TO_OBSERVATION_AGAIN:
                ...
                drive.update();
                autoState = BlindManSensing1_2.AutoState.LIFT_SLOWLY_SECOND;
                break;
            case LIFT_SLOWLY_SECOND:
                if ((Math.abs(drive.getPoseEstimate().getX() - 20) <= 2) && (Math.abs(drive.getPoseEstimate().getY() + 39) <= 2)
                    ...
                    testTimer.reset();
                    isTiltIncrementing = true;
                    autoState = BlindManSensing1_2.AutoState.EXTEND_TO_SECOND_SPECIMEN;
                    break;
                }
                break;
            case EXTEND_TO_SECOND_SPECIMEN:
                if (testTimer.seconds() >= 1) {
                    ...
                    autoState = BlindManSensing1_2.AutoState.GRAB_SECOND_SPECIMEN;
                    break;
                }
                break;
            case GRAB_SECOND_SPECIMEN:
                if ((Math.abs(slideMotor.getCurrentPosition()- 370) <= 10)) {
                    if (distance <= 1.8) {
                        intake_claw_servo.setPosition(Claws_closed);
                        grabTimer.reset();
                        autoState = BlindManSensing1_2.AutoState.DRIVE_TO_SCORE_SECOND_SPECIMEN;
                        break;
                    } else if (isTiltIncrementing) {
                        tiltMotor.setTargetPosition(tiltMotor.getCurrentPosition() + 20);
                        isTiltIncrementing = false;
            ...
            case DRIVE_TO_SCORE_SECOND_SPECIMEN:
                if (grabTimer.seconds() >= .3 ) {
                    ...
                    if (tiltMotor.getCurrentPosition() >= 780) {
                        ...
                        autoState = BlindManSensing1_2.AutoState.SCORE_SECOND_SPECIMEN;
                    }
                    break;
                }
                break;
            case SCORE_SECOND_SPECIMEN:
                if ((Math.abs(drive.getPoseEstimate().getX() - 20) <= 2.5) && (Math.abs(drive.getPoseEstimate().getY() + 2) <= 2.5)
                    ...
                    if ((Math.abs(tiltMotor.getCurrentPosition() - 1070) <= 10) &&
                        ...
                        autoState = BlindManSensing1_2.AutoState.RETRACT_SECOND;
                        break;
                    }
                }
                break;
            case RETRACT_SECOND:
                if (dunkAgainTimer.seconds() >=.3) {
                    ...
                    if (dunkAgainTimer.seconds() >= 0.8) {
                        intake_claw_servo.setPosition(Claws_open);
                        autoState = BlindManSensing1_2.AutoState.PARK;
                        break;
                    }
                }
                break;
            case PARK:
                ...
                drive.followTrajectorySequenceAsync(grabSecondSpecimen);
                drive.update();
                autoState = BlindManSensing1_2.AutoState.FINISH;
                break;
            case FINISH:
                if ((Math.abs(drive.getPoseEstimate().getX() - 20) <= 2) && (Math.abs(drive.getPoseEstimate().getY() + 39) <= 2)) {
